function refreshWeather(response) {
    ...
    // Extract only the city name from the response if it's returning more detailed information (e.g., "New York County").
    let cityName = response.data.city;
    if (cityName.includes("County")) {
        cityName = cityName.split(" County")[0];  // Removes "County" from the name
    }
...
function displayWeeklyForecast(response) {
    ...
    forecastDays.forEach((day) => {
        ...
        // Display days in shortened format (Mon, Tue, etc.)
